...
- ALWAYS: `anthropics/claude-code-action` is tracked by a dedicated Renovate rule with a weekday update cadence while remaining full-SHA pinned to concrete release tags, so trusted beta consumers on `outcomeeng/gh-actions@main` receive reviewed upstream fixes from this repository and production consumers receive them through pinned reusable-workflow update PRs ([audit])
