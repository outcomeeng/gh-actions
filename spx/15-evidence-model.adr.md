...
- NEVER: an assertion carries `[eval]` — this product ships no LLM-graded output of its own; the agent's review behavior is governed by `plugins/spx/21-spec-tree.enabler/68-reviewing.enabler/reviewing.md` and cross-referenced, so the `[audit]`, `actionlint`/`shellcheck` conformance, and `scripts/`-scoped `[test]` lanes above cover every assertion ([audit])
