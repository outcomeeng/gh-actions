...
2. Either (a) mention `@claude` (fires `claude-repo.yml`) or `@spec-tree` (fires `spec-tree-repo.yml`) on an issue or PR in this repo / open a PR (the `*-repo.yml` callers fire the reusables locally), or (b) update an external test repo to use `@branch-name` instead of `@main`
...
3. **Top-level `permissions: {}`** on every **leaf** reusable workflow (`claude.yml`, `claude-code-review.yml`) and on `ci.yml`. Jobs explicitly grant the narrow permissions they need, subject to the caller workflow's maximum grant. The wrapper workflows (`spec-tree.yml`, `spec-tree-review.yml`) deliberately omit a top-level `permissions:` block because workflow_call's permission model is an intersection: setting `permissions: {}` on a wrapper zeros the chain and the called workflow's per-job requests fail with `startup_failure`. The caller workflow's permissions set the maximum; the leaf reusable's per-job grants do the actual narrowing.
...
5. **Secrets reach the runner via the action's `with:` inputs or via job-level `env:` blocks** — never interpolated into a `run:` script body. The workflow `39ad2e8` removed the one remaining `${{ inputs.custom_prompt }}` interpolation; the pattern is enforced going forward by actionlint + shellcheck.
